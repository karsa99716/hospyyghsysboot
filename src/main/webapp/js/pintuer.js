$(function(){
	...
	$('.banner').each(function(){
		...
		var shixitongs=e.attr("data-shixitong");
		var shixitongs_s=e.attr("data-small");
		var shixitongs_m=e.attr("data-middle");
		var shixitongs_b=e.attr("data-big");
		var num=e.find(".carousel .shixitong").length;
		var win=$(window).width();
		var i=1;

		if(interval==null){interval=5};
		if(shixitongs==null || shixitongs<1){shixitongs=1};
		if(shixitongs_s!=null && win>760){shixitongs=shixitongs_s};
		if(shixitongs_m!=null && win>1000){shixitongs=shixitongs_m};
		if(shixitongs_b!=null && win>1200){shixitongs=shixitongs_b};

		var shixitongWidth=Math.ceil(e.outerWidth()/shixitongs);
		var page=Math.ceil(num/shixitongs);
		e.find(".carousel .shixitong").css("width",shixitongWidth+ "px");
		e.find(".carousel").css("width",shixitongWidth*num + "px");
		
		var carousel=function(){
			i++;
			if(i>page){i=1;}
			$showbanner(e,i,shixitongs,num);
		};
		...
		if(pointer!=0 && page>1){
			...
			pager.find("li").click(function(){
				$showbanner(e,$(this).val(),shixitongs,num);
			});
	...
	$showbanner=function(e,i,shixitongs,num){
		var after=0,leftx=0;
		leftx = - Math.ceil(e.outerWidth()/shixitongs)*(shixitongs)*(i-1);
		if(i*shixitongs > num){after=i*shixitongs-num;leftx= - Math.ceil(e.outerWidth()/shixitongs)*(num-shixitongs);};
